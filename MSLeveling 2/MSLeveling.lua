...
local MSG = {
	REPLY_HELP = "Whisper your role and aura to sign up, e.g. 'tank aura', 'heal no aura' or 'dps'. Addon: " .. ADDON_LINK,
	REPLY_OK = "You are on the queue list. If there is room for your role you will be invited.",
	...
	FAREWELL = "Thanks for joining the group, good luck with your rolls! If you want to join again you can grab the addon here: " .. ADDON_LINK,
}
...
local IsInsideManastorm, IsManastormZone = nil, nil
...
-- Returns a reason to auto-reply to recruiting whispers immediately (raid full
-- or inside a Manastorm), regardless of the auto-reply setting, or nil.
local function WhisperBlockReason()
	if GetNumRaidMembers() >= MAX_TOTAL then
		return MSG.REJECT_GROUP_FULL
	end
	...
	if IsInsideManastorm() or IsManastormZone() then
		return MSG.REJECT_IN_MANASTORM
	end
	return nil
end
...
if title then
		title:SetText("|cff66b3ffMS Leveling 2.07|r" .. (collecting and " |cff7d7ffd[Collecting...]|r" or ""))
	end
end
...
local function TryAutoInvite(name, fromChannel)
	if IsBlocked60(name) then
		if not fromChannel then
			pcall(SendChatMessage, "You are blocked from this group. Sorry!", "WHISPER", nil, name)
		end
		return false
	end
...
function SortGroups()
	...
	local function finish()
		...
		if #auraParts > 0 then
			SendChatMessage("MSLeveling: {triangle}" .. table.concat(auraParts, ", "), "RAID")
		end
		if #healParts > 0 then
			SendChatMessage("MSLeveling: {square}" .. table.concat(healParts, "; "), "RAID")
		end
		SendChatMessage("MSLeveling: {circle}Tanks: " .. joinList(tankList), "RAID")
...
function HandleWhisper(msg, author)
	...
	if blockReason then
		...
		if not rejectSent[name] or now - rejectSent[name] >= 60 then
			rejectSent[name] = now
			pcall(SendChatMessage, blockReason, "WHISPER", nil, name)
		end
		return
	end
...
WarnWipeForLevel60 = function(newLevel)
	...
	newLevel = tonumber(newLevel)
	if not newLevel or newLevel < 60 then
		return
	end
	...
	for name, info in pairs(flaggedPlayers) do
		if info and info.reason == FLAG.LVL60 and present[name] then
			local already = false
	...
	local warningKey = table.concat(shortNames, ",")
				...
				if lvl and lvl >= 59 then
					if lvl >= 60 then
						...
						if not warningSeen[key] then
							...
							local rec = BuildRoleInfo()[n]
							local roleTag = (rec and rec.aura) and "(Aura)"
								or (rec and rec.role == "Tank") and "(Tank)"
								or (rec and rec.role == "Heal") and "(Heal)"
								or "(Just a DPS dont worry)"
							SendChatMessage("[MSL] " .. n .. " reached LEVEL 60 " .. roleTag .. ". Removing from group.", "RAID_WARNING")
						end
...
function DoFeatureEnter()
	if AutoEnterManastorm then
		AutoEnterManastorm()
	else
		print(PREFIX .. "Enter Manastorm: addon still initializing.")
	end
end
...
function DoFeatureDisband()
	...
	local function TryReinvite()
		...
		local function FinishReinvite()
			...
			SendChatMessage(PREFIX .. "Re-invite complete: " .. okCount .. " player(s) re-invited.", "RAID_WARNING")
...
title:SetText("|cff66b3ffMS Leveling 2.07|r")
...
CheckAutoOff = function()
	...
	if tot >= MAX_TOTAL then
		if not fullWiped then
			...
			if IsRaidLeader() or IsRaidOfficer() then
				print(PREFIX .. "Raid full (15/15), waiting for the last player to accept to sort the groups.")
				SendChatMessage("Raid full.", "RAID")
				SendChatMessage("Last invite sent, waiting for them to accept before sorting the groups.", "RAID")
			end
		end
		if db.autoLFM then
			db.autoLFM = false
	...
	if GetNumRaidMembers() >= MAX_TOTAL then
		if not autoSortDone then
			autoSortDone = true
			if IsRaidLeader() or IsRaidOfficer() then
				print(PREFIX .. "Raid complete (15/15), sorting groups automatically.")
				SortGroups()
			end
		end
	else
...
local function ManastormLevelOneSelected(dropdown)
	...
	return tonumber((ManastormFrameText(dropdown) or ""):match("[Ll]evel%s*(%d+)")) == 1
end

local function FindManastormLevelOneButton()
	local function IsLevelOneClickable(button)
		if type(button) ~= "table" or type(button.Click) ~= "function" then
			return false
		end
		return tonumber((ManastormFrameText(button) or ""):match("[Ll]evel%s*(%d+)")) == 1
	end
	...
	if type(buttons) == "table" then
		for _, button in pairs(buttons) do
			if IsLevelOneClickable(button) then
				return button
			end
		end
	end
	local roots = {
		...
		_G.ManastormQueueFrameRightPanelLevelDropDown,
		levelList,
		scrollFrame,
	}
	local seen = {}
	local function FindInChildren(frame, depth)
		...
		seen[frame] = true
		if IsLevelOneClickable(frame) then
			return frame
		end
		if type(frame.GetChildren) == "function" then
			for _, child in ipairs({ frame:GetChildren() }) do
	...
	-- Standard UIDropDownMenu popups spawn their entries under UIDROPDOWNMENU.
	if _G.UIDROPDOWNMENU then
		local found = FindInChildren(_G.UIDROPDOWNMENU, 0)
		if found then return found end
	end
	-- Last resort: scan every Manastorm-named frame in _G for a shown Level 1 clickable.
	for k, v in pairs(_G) do
		if type(v) == "table" and type(v.GetObjectType) == "function"
			and tostring(k):match("[Mm]anastorm")
			and (type(v.IsShown) ~= "function" or v:IsShown())
			and IsLevelOneClickable(v) then
			return v
		end
	end
	return nil
...
GetManastormEnterState = function()
	...
	if not ManastormLevelOneSelected(dropdown) then
		-- Ascension hides the level entries behind a drop-down that must be opened
		-- first. Clicking it is retried every 0.5s by the poll loop; once the menu
		-- is up, the Level 1 entry becomes clickable and this converges.
		local levelButton = FindManastormLevelOneButton()
		if not levelButton and dropdown and type(dropdown) == "table" then
			if type(dropdown.Show) == "function" and type(dropdown.IsShown) == "function"
				and not dropdown:IsShown() then
				pcall(dropdown.Show, dropdown)
			end
			if type(dropdown.Click) == "function" then
				pcall(dropdown.Click, dropdown, "LeftButton")
			elseif type(dropdown.Toggle) == "function" then
				pcall(dropdown.Toggle, dropdown)
			end
			levelButton = FindManastormLevelOneButton()
		end
		if levelButton then
			pcall(levelButton.Click, levelButton, "LeftButton")
		end
		if not ManastormLevelOneSelected(dropdown) then
			return "level"
		end
...
function PrintManastormApi()
	...
	if type(api) == "table" then
		...
		for k, v in pairs(api) do
			table.insert(names, tostring(k) .. "(" .. type(v) .. ")")
		end
		table.sort(names)
		print("  C_Manastorm: " .. table.concat(names, ", "))
			...
			if ok and type(levels) == "table" then
				...
				for _, value in pairs(levels) do
					if type(value) == "table" then
						table.insert(values, tostring(value.level or value.id or "?"))
					else
		...
		if type(api.CanEnter) == "function" then
			...
			print("  CanEnter(1): " .. (ok and tostring(allowed) or "ERROR")
				.. (reason ~= nil and ", reason=" .. tostring(reason) or ""))
		end
		if type(api.Enter) == "function" then
			print("  C_Manastorm.Enter: available (info only, not used)")
		else
			print("  C_Manastorm.Enter: NOT available")
		end
		if type(api.IsInManastorm) == "function" then
			local ok, inside = pcall(api.IsInManastorm)
			print("  IsInManastorm(): " .. (ok and tostring(inside) or "ERROR"))
		end
	else
	...
	print("  Level 1 dropdown selected: " .. tostring((function()
		return ManastormLevelOneSelected(_G.ManastormQueueFrameRightPanelLevelDropDown)
	end)()))
	print("  Level 1 button found: " .. tostring(FindManastormLevelOneButton() ~= nil))
	local queueFrame = _G.ManastormQueueFrame
	if not queueFrame then
		print("  Queue tree: ManastormQueueFrame not loaded.")
	else
		print("  Queue tree:")
		local seen = {}
		local lines = 0
		local function Walk(frame, depth)
			if not frame or seen[frame] or depth > 6 or lines >= 120 then
				return
			end
			seen[frame] = true
			local name = type(frame.GetName) == "function" and frame:GetName() or nil
			local ot = type(frame.GetObjectType) == "function" and frame:GetObjectType() or "?"
			local text = ManastormFrameText(frame)
			local shown = type(frame.IsShown) == "function" and frame:IsShown() and "shown" or ""
			lines = lines + 1
			print("    " .. string.rep("  ", depth) .. (name or "<anon>")
				.. " [" .. ot .. "]" .. (shown ~= "" and " " .. shown or "")
...
local function TryEnterManastormApi()
	local api = _G.C_Manastorm
	if type(api) ~= "table" or type(api.Enter) ~= "function" then
		return false
	end
	if type(api.CanEnter) == "function" then
		local ok, allowed = pcall(api.CanEnter, 1)
		if ok and allowed == false then
			print(PREFIX .. "Mana Storm Level 1 is not enterable right now.")
			return false
		end
	end
	local ok = pcall(api.Enter, 1)
	if ok then
		print(PREFIX .. "Requesting entry to Mana Storm Level 1 via C_Manastorm...")
		return true
	end
	print(PREFIX .. "C_Manastorm.Enter is blocked here; using the Mana Storm panel.")
	return false
end

local MANASTORM_OPEN_COMMANDS = { "MANASTORM", "MANASTORMS", "MSTORM" }

local function TryOpenManastormPanel()
	local function runHandler(fn)
		if not fn then return false end
		return pcall(fn, "")
	end
	if SlashCmdList then
		for _, key in ipairs(MANASTORM_OPEN_COMMANDS) do
			if SlashCmdList[key] and runHandler(SlashCmdList[key]) then
				return true
			end
		end
		for key, fn in pairs(SlashCmdList) do
			local n = tostring(key):lower()
			if n:find("manastorm", 1, true) or n:find("mstorm", 1, true) then
				if runHandler(fn) then
					return true
				end
			end
		end
	...
	for k, v in pairs(_G) do
		if type(v) == "string" and tostring(k):match("^SLASH_") then
			local s = tostring(v):lower()
			if s:find("manastorm", 1, true) or s:find("mstorm", 1, true) then
				local base = tostring(k):match("^SLASH_(.+)%d+$")
				if base and SlashCmdList and SlashCmdList[base] and runHandler(SlashCmdList[base]) then
					return true
				end
			end
		end
	...
	for k, v in pairs(_G) do
		if type(v) == "function" then
			local n = tostring(k)
			if n:find("[Mm]anastorm") and (n:find("Toggle", 1, true) or n:find("Show", 1, true) or n:find("Open", 1, true)) then
				if runHandler(v) then
					return true
				end
			end
		end
...
local function StepEnterManastorm()
	if not msEnterState then return end
	msEnterTries = msEnterTries + 1
	if msApiPending then
		if IsInsideManastorm() or IsManastormZone() then
			msEnterState = nil
			msApiPending = false
			print(PREFIX .. "Entered Mana Storm Level 1.")
			return
		end
		if msEnterTries > 12 then
			msApiPending = false
			print(PREFIX .. "API entry not confirmed; using the Mana Storm panel instead.")
		else
			After(0.5, StepEnterManastorm)
			return
		end
	end
	...
	if state == "ready" and enterButton then
		...
		if ok then
			...
			print(PREFIX .. "Enter button click was blocked. Try again outside combat.")
		end
		return
	end
	if state == "noload" then
		if not msEnterOpened then
			msEnterOpened = true
			if TryOpenManastormPanel() then
				print(PREFIX .. "Opening the Mana Storm panel automatically...")
			else
				print(PREFIX .. "Could not find the Mana Storm slash command. Open the panel once manually.")
			end
		end
	elseif state == "level" then
		if not msEnterWarned then
			msEnterWarned = true
			print(PREFIX .. "Selecting Level 1 in the Mana Storm panel...")
		end
	elseif state == "disabled" then
		if not msEnterWarned then
			msEnterWarned = true
			print(PREFIX .. "Waiting for the Enter button to become available...")
		end
	end
	if msEnterTries > 90 then
		msEnterState = nil
		msEnterOpened = false
		msEnterWarned = false
		msApiPending = false
		print(PREFIX .. "Enter Manastorm: could not complete automatically. Run /mslv api to inspect.")
		return
	end
	After(0.5, StepEnterManastorm)
end

function AutoEnterManastorm()
	if msEnterState then
		print(PREFIX .. "Enter Manastorm: already in progress.")
		return
	end
	if not IsInRaid() then
	...
	if InCombatLockdown() then
		print(PREFIX .. "Enter Manastorm requires you to be out of combat. Click again after combat ends.")
		return
	end
	msEnterState = true
	msEnterTries = 0
	...
	msEnterWarned = false
	msApiPending = false
	print(PREFIX .. "Enter Manastorm: preparing automatically...")
	if TryEnterManastormApi() then
		msApiPending = true
	end
	StepEnterManastorm()
end
...
for i, def in ipairs(featureDefs) do
	...
	b:SetText(def[1])
	if def[2] == "Ready" then
		b:SetScript("OnClick", DoFeatureReady)
	elseif def[2] == "EnterMS" then
		b:SetScript("OnClick", function()
			AutoEnterManastorm()
		end)
	elseif def[2] == "Disband" then
		b:SetScript("OnClick", DoFeatureDisband)
	elseif def[2] == "Anti" then
		b:SetScript("OnClick", function(self)
			db.antileech = not db.antileech
			self:SetText(db.antileech and "Anti-leech" or "Anti-leech*")
