...
- When only an Aura is missing, the LFM says `LF1M {triangle}1 Aura (Any role)` and it auto-invites **any role with an aura** (tank/heal/dps + aura).
- **Aura slot reservation:** whenever auras are missing, DPS slots are reserved for the aura carriers. The number of reserved DPS slots is always **missing auras + 1** (3 auras missing → 4 DPS slots, 2 → 3, 1 → 2; with all 3 auras, 1 slot stays reserved for **any role with an aura**). Plain DPS below the reserved slots are auto-invited normally; the reserved slots only accept players **with an aura**.
- A Tank or Heal **with an aura** is auto-invited into a reserved DPS slot **even if the tank/heal quota is full**.
